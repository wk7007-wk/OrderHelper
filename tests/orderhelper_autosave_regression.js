...
function reset(stock = 10) {
  ...
  api.setEntriesForCheck([{ id: 'enter-row', entryKey: 'enter-row', name: item.name, zone: '', stock }]);
  api.markInventoryMutationForCheck();
  api.saveLocalForCheck('draft');
}

function updateDraft(stock) {
  api.setEntriesForCheck([{ id: 'enter-row', entryKey: 'enter-row', name: item.name, zone: '', stock }]);
  api.markInventoryMutationForCheck();
  api.saveLocalForCheck('draft');
}
...
async function verifyEnterPairAndExactBody() {
  ...
  assert.strictEqual(api.saveStatusForCheck().localDirty, false, 'matching state/inventory revisions may clear dirty state');
}
...
async function verifyRetryDoesNotCaptureLaterDraft() {
  ...
  await waitFor(() => !api.saveStatusForCheck().saveInFlight && fakeTimers.timers.size === 1, 'retry timer');
  ...
  assert(calls.slice(2).every(call => stockFromBody(call.body) === 10), 'retry must replay old Enter body, not unentered 20');
  assert.strictEqual(api.entriesForCheck()[0].stock, 20, 'new unentered local draft must remain 20');
  assert.strictEqual(api.saveStatusForCheck().localDirty, true, 'success of older revisions must not clear the newer draft');
  fakeTimers.restore();
}
...
async function verifySecondEnterQueuesLatest() {
  ...
  updateDraft(20);
  api.confirmCurrentSave('enter');
  const queue = api.readConfirmedSaveQueue();
  assert.strictEqual(stockFromBody(queue.active.body), 10, 'active commit must remain first Enter value');
  assert.strictEqual(stockFromBody(queue.queued.body), 20, 'second Enter must queue latest confirmed value');
  firstPending.splice(0).forEach(resolve => resolve(response(true)));
  await waitFor(() => calls.length === 4 && !api.readConfirmedSaveQueue().active && !api.saveStatusForCheck().saveInFlight, 'queued Enter');
  ...
  assert.strictEqual(api.saveStatusForCheck().localDirty, false, 'latest queued success may clear matching revisions');
}
...
async function verifyPartialFailureRetriesBothExactTargets() {
  ...
  failHistory = false;
  fakeTimers.runOnlyTimer();
  await waitFor(() => calls.length === 4 && !api.saveStatusForCheck().saveInFlight, 'partial retry');
  assert(calls.every(call => call.body === firstBody), 'partial retry must replay one exact body to both targets');
  assert(calls.filter(call => call.url.includes('/history/')).every(call => call.url === historyUrl), 'retry must retain captured history date path');
  fakeTimers.restore();
}
...
async function verifyRecoveryAndFailClosedStorage() {
  ...
  await waitFor(() => calls.length === 2 && !api.saveStatusForCheck().saveInFlight, 'pending reload');
  assert(calls.every(call => call.body === commit.body), 'reload must replay exact persisted confirmed body');

  reset(50);
  calls.length = 0;
  const originalSetItem = sandbox.localStorage.setItem;
  sandbox.localStorage.setItem = (key, value) => {
    if (key === api.confirmedQueueKeyForCheck) throw new Error('quota');
  ...
  api.resetConfirmedSaveMachineForCheck();
  storage.set(api.confirmedQueueKeyForCheck, '{broken');
...
(async () => {
  ...
  console.log('OrderHelper integrated Enter-confirmed autosave VM regression OK');
})().catch(error => {
  console.error(error);
  process.exitCode = 1;
});
