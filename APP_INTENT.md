...
- `orderhelper_usage_inference.py --upload`는 `/unitInference/latest` 리포트만 갱신한다. `/sfaActualHistory` 백필은 별도 명시 플래그가 있을 때만 허용한다.
...
- 전달: 웹 URL 반영 확인
- 최신 기준: `20260617-1` / 하루사용량 계산은 수동값 기준, 실사용량 분석은 참고 표시 전용 / 출력순서는 최초 SFA 순서 고정 / 라이브 `https://wk7007-wk.github.io/OrderHelper/`
