...
- 전달: 웹 URL 반영 확인
- 최신 배포판 기준: `20260717-19` / `0717.0725` / separate stock-unit price from one-order-unit price. 환산 품목은 재고 1단위 가격과 발주 1단위 가격을 함께 명시하고, 예상액 계산은 기존의 추천 발주수량 × 발주 1단위 가격을 유지한다. 이름 연결·숫자 보정 2단계, `연결 안 함`, order-first grid, compact save status, exact hash 자동 진입, 저장/CAS/수동값 보호는 유지한다.
- 완료 포인터: `20260717-19 Separate stock-unit and order-unit prices`; 이전 포인터 `20260717-18 Simplify alias editor to name + quantity`, `20260717-17 Persist explicit order-name unlink`.
