- 2026-09-04: APP_VERSION 0904.0456 clearer factory_https_unreachable phone Tailscale/MagicDNS hint.
- 2026-09-04: APP_VERSION 0904.0453 forces MagicHTTPS factory PUT; never prefer gist write; error factory_https_unreachable.
...
- 발주 품목 출력순서는 SFA 즐겨찾기 `sfaSeq`다. 저장되어 있던 사용자 지정 `outputOrder`는 읽어도 적용하지 않는다. 뼈한마리는 냉동 블록 다음·떡볶이/필크런치 앞, 치킨무(벌크)는 배달용보다 앞이다.
- 임시 수신 칩 `CLI-OK-0902A`는 `APP_VERSION` 옆에 두고 SFA 정렬 모드에 묶지 않는다. SFA 사이트 UI는 바꾸지 않는다.
- `dailySales`/`dailySalesAt`/`salesWeights` 길이는 `orderDays`와 같다.
...
- 정적 프론트 인증은 등록창 실접속으로 확인된 개인폰 1개(`personal-phone`)·공장PC 일반 브라우저 1개(`factory-pc`)·공장PC Codex 앱 내 브라우저 1개(`factory-pc-codex-in-app`, 2026-07-17 등록)·메인PC 1개(`agent-pc`, `user_this_pc`, 2026-08-23 등록)의 exact SHA-256 hash 네 개만 거리·PIN·네트워크와 무관하게 자동 진입한다. 개인폰과 메인PC는 이 목록에 이미 있으므로 PIN 없이 들어간다. 미등록 브라우저는 PIN(또는 동일 SHA-256 비밀번호)만으로 잠금 해제하며, 올바른 PIN 뒤에 `verifyAuthFactor()`/GPS/데스크탑 등록을 차단 조건으로 두지 않는다. 이름/label이나 `/desktopAccess` candidate row만으로는 passwordless 근거가 되지 않는다.
...
## 운영 주의 · SFA 정주문 카운트다운 (일요/월 새벽)
- SFA 화면의 "주문시작까지 N시간" 카운트다운은 세션/캐시에 따라 어긋날 수 있다.
- 미개방·미수금으로 오인하지 말 것. 막히면 **재로그인 후 재시도**.
- 증거(2026-09): 카운트다운 오차→블락 오인, 재로그인 후 미수금 표시 소멸.
