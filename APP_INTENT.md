...
- 전달: 웹 URL 반영 확인
- 최신 배포판 기준: `20260717-12` / `0717.0514` / compact one-line save status. 연속 재고 입력의 저장 로그는 직원 화면에서 제거하고 상단 저장 상태만 유지한다. 공장PC Codex 앱 내 브라우저 exact hash 자동 진입, employee-safe auth copy, low-latency stock input·소수 환산 후보 방어·저장/CAS/수동값 보호는 그대로 유지한다.
- 완료 포인터: `20260717-12 Hide employee-facing save history`; 이전 포인터 `20260717-11 Hide inactive registration badge`, `20260717-10 Hide employee-facing auth diagnostics`.
