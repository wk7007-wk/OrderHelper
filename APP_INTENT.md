...
- 계산식과 사용자 입력 기록은 화면 정리 때문에 숨기거나 잃지 않는다. 재고 숫자를 치는 동안에는 그 칸 값과 필요량·추천발주 숫자만 고친다. 정렬, 커서 이동, 원격 저장, 분석카드 재생성은 하지 않는다. Enter 또는 모바일 다음(아직 그 칸에 있을 때)은 기존 행을 그 자리에서 한 번 정렬한 뒤 다음 미입력 재고칸으로 커서를 한 번만 옮긴다. 그 뒤에 다시 정렬하거나 커서를 다시 두지 않고, tbody를 통째로 다시 만들지 않는다. 이미 다른 칸에 있으면 정렬만 하고 커서는 그대로 둔다. 완료 전 `input` draft와 다른 필드의 일반 blur/change는 원격 전송하지 않는다.
