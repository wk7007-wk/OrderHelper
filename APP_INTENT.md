...
- 정적 프론트 인증은 2026-07-17 등록창 실접속으로 확인된 개인폰·공장PC token의 exact SHA-256 hash 두 개만 거리·PIN·네트워크와 무관하게 자동 진입한다. 메인PC는 passwordless 대상이 아니며 기존 PIN+거리/승인 factor를 사용한다. 이름/label이나 `/desktopAccess` candidate row만으로는 passwordless 근거가 되지 않는다.
