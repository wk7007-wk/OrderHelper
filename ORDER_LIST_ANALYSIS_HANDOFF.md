...
- 2026-05-29 보강: `index.html` `APP_VERSION=20260529-6`. `advanceStockInput()` 공통 헬퍼와 `change` fallback을 추가해, active가 현재 재고칸이면 다음 재고칸으로 이동하게 함. 추가로 입력 확정 후 `render()`를 다시 실행해 `미입력 → 입력완료 → 숨김` 정렬을 즉시 반영하고, 정렬로 DOM이 바뀐 뒤에도 원래 가야 할 다음 재고칸을 다시 focus/select 한다. `구역/여유/일사용/행 버튼`은 탭 순서에서 제외해 모바일 기본 다음 이동도 재고칸 중심으로 흐르게 함.
